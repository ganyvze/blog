...
string U2G(const string& s) {
    ...
    if (s.empty()) return "";
    int l = MultiByteToWideChar(CP_ACP, 0, s.c_str(), -1, 0, 0);wchar_t* w = new wchar_t[l + 1];MultiByteToWideChar(CP_ACP, 0, s.c_str(), -1, w, l);l = WideCharToMultiByte(CP_UTF8, 0, w, -1, 0, 0, 0, 0);char* c = new char[l + 1];WideCharToMultiByte(CP_UTF8, 0, w, -1, c, l, 0, 0);string r(c);delete[] w;delete[] c;return r;
}struct ChatMessage {
...
struct IncomingAssembly {
    string sender_ip;string sender_name;string target_ip;bool is_private;int total_parts;vector<string> parts;vector<char> received;long long updated_ms;
};
struct UploadedFile {
...
long long now_ms() {
    ...
    string search = "\"" + key + "\":\"";size_t p = body.find(search);
...
}string get_request_path(const string& req) {
...
}string get_query_param(const string& path, const string& key) {
    size_t q = path.find('?');if (q == string::npos) return "";
    ...
    while (getline(iss, line)) {
        ...
        string header_name = to_lower_copy(trim_copy(line.substr(0, colon)));
        if (header_name == lower_key) return trim_copy(line.substr(colon + 1));
    }return "";
}string detect_local_ip() {
    if (!API.ghn || !API.ghb) return "127.0.0.1";
    char host[256] = {0};
    if (API.ghn(host, sizeof(host) - 1) != 0) return "127.0.0.1";
    hostent* ent = API.ghb(host);
    if (!ent) return "127.0.0.1";
...
}void send_simple_response(SOCKET c, const string& status, const string& content_type, const string& body) {
...
}void send_not_found(SOCKET c) {
    send_simple_response(c, "404 Not Found", "text/plain; charset=utf-8", "Not Found");
    ...
    if (msg_history.size() > MAX_MSG_HIST) msg_history.erase(msg_history.begin());
    ++state_cursor;lk.unlock();state_cv.notify_all();
    ...
    bool changed = false;unique_lock<mutex> lk(state_mtx);OnlineUser& u = online_users[ip];
    if (u.ip.empty()) {
    ...
    if (changed) {++state_cursor;lk.unlock();state_cv.notify_all();}
}string build_packet(const string& type, const vector<pair<string, string>>& headers, const string& body) {
    ...
    while (getline(iss, line)) {
        ...
        headers[trim_copy(line.substr(0, eq))] = line.substr(eq + 1);
    ...
    }string packet = build_packet("HEARTBEAT", {
        {"SENDER_ID", to_string(my_id)},
        {"NAME", cur_name},
        {"HTTP_PORT", "28081"}
    }, "");
    ...
    size_t total_parts = max<size_t>(1, (content.size() + UDP_SAFE_PAYLOAD - 1) / UDP_SAFE_PAYLOAD);
...
}void finalize_incoming_message(const string& sender_ip, const string& sender_name, const string& target_ip, unsigned long long seq, bool is_private, const string& content) {
    ...
    if (is_private) {
        ...
        } else {delivered_private[key] = now_ms();}
    }if (!duplicate) append_chat_message(sender_ip, sender_name, target_ip, content, false);
    ...
    string key = sender_ip + "#" + to_string(seq);bool ready = false;string merged;
    {
        ...
        if (!item.received[part]) {
            item.parts[part] = body;
            item.received[part] = 1;
        }ready = true;
        for (int i = 0; i < item.total_parts; ++i) {
            if (!item.received[i]) {ready = false;break;}
        }if (ready) {
            for (int i = 0; i < item.total_parts; ++i) merged += item.parts[i];
            incoming_assemblies.erase(key);
        }
    ...
    if (p3 != string::npos) {
        sender_name = raw.substr(p1 + 1, p2 - p1 - 1);target_ip = raw.substr(p2 + 1, p3 - p2 - 1);content = raw.substr(p3 + 1);
    } else {
    ...
    while (true) {
        ...
        if (parse_packet(raw, headers, body)) {
            int sender_id = parse_int(headers.count("SENDER_ID") ? headers.at("SENDER_ID") : "-1", -1);
    ...
    while (true) {broadcast_heartbeat();Sleep((DWORD)HEARTBEAT_INTERVAL_MS);}
    ...
    while (true) {
        Sleep(200);vector<PendingReliableMessage> resend_list;vector<PendingReliableMessage> fail_list;long long now = now_ms();
        {
            ...
            for (auto it = pending_private.begin(); it != pending_private.end();) {
                if (now - it->second.last_send_ms < ACK_TIMEOUT_MS) {
                    ++it;continue;
    ...
    while (true) {
        Sleep(1000);long long now = now_ms();bool state_changed = false;
        {
            ...
            for (auto& kv : online_users) {
                if (kv.second.online && now - kv.second.last_seen_ms > USER_OFFLINE_MS) {
                    kv.second.online = false;state_changed = true;
                }
            }if (state_changed) ++state_cursor;
        }if (state_changed) state_cv.notify_all();
        {
            lock_guard<mutex> lk(udp_mtx);
            for (auto it = incoming_assemblies.begin(); it != incoming_assemblies.end();) {
                if (now - it->second.updated_ms > ASSEMBLY_EXPIRE_MS) it = incoming_assemblies.erase(it);
                else ++it;
...
}bool parse_multipart_file(const string& content_type, const string& body, string& filename, string& file_data) {
    string lower = to_lower_copy(content_type);size_t bpos = lower.find("boundary=");
    ...
    string marker = "--" + boundary;size_t pos = body.find(marker);
    while (pos != string::npos) {
        pos += marker.size();
        if (pos + 2 <= body.size() && body.compare(pos, 2, "--") == 0) break;
        if (pos + 2 <= body.size() && body.compare(pos, 2, "\r\n") == 0) pos += 2;
        size_t part_header_end = body.find("\r\n\r\n", pos);
        if (part_header_end == string::npos) break;
        string part_headers = body.substr(pos, part_header_end - pos);string lower_headers = to_lower_copy(part_headers);size_t data_start = part_header_end + 4;size_t next_boundary = body.find("\r\n" + marker, data_start);
        ...
        if (fn_pos != string::npos) {
            ...
            filename = part_headers.substr(value_start, value_end - value_start);file_data = body.substr(data_start, next_boundary - data_start);return !filename.empty();
        }pos = next_boundary;
    }return false;
    ...
    for (char ch : disk_name) {
        unsigned char c = (unsigned char)ch;if (!(isalnum(c) || ch == '.' || ch == '_' || ch == '-')) return false;
    }return true;
    ...
    string cur_name;
    {
        lock_guard<mutex> lk(state_mtx);
        cur_name = sanitize_name(my_name);
    }string actual_dest = dest_ip.empty() ? "255.255.255.255" : dest_ip;append_chat_message(my_ip, cur_name, actual_dest, text, true);unsigned long long seq = next_msg_seq.fetch_add(1);vector<string> packets = build_message_packets(seq, cur_name, actual_dest, text);
...
<head>
    ...
    <style>
        * { box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
        ...
        .contact-item { padding: 12px 15px; display: flex; align-items: center; cursor: pointer; position: relative; }
        ...
        .contact-avatar { width: 40px; height: 40px; background: #07c160; color: #fff; border-radius: 4px; display: flex; align-items: center; justify-content: center; font-size: 20px; margin-right: 12px; flex-shrink: 0; font-weight: bold; }
        .contact-avatar.global { background: #ff9800; }
        ...
        .status-dot.online { background: #07c160; box-shadow: 0 0 0 4px rgba(7, 193, 96, 0.12); }
        .unread-dot { color: #f44336; font-weight: bold; font-size: 12px; white-space: nowrap; }
        ...
        .chat-area { flex-grow: 1; padding: 20px; overflow-y: auto; display: flex; flex-direction: column; transition: background-color 0.2s, outline 0.2s; }
        .chat-area.dragover { background: #eef8f1; outline: 2px dashed #07c160; outline-offset: -10px; }
        ...
        .msg-others .bubble { background-color: #fff; color: #000; border: 1px solid #eaeaea; }
        ...
        .file-link { color: #576b95; text-decoration: none; font-size: 13px; }
        ...
        .input-area textarea { flex-grow: 1; padding: 10px 15px; border: none; resize: none; font-size: 15px; outline: none; background: #f5f5f5; }
        .input-action { padding: 10px 20px; display: flex; justify-content: flex-end; }
        ...
        .settings-card input:focus { border-color: #07c160; }
        .settings-card button { width: 100%; padding: 10px; background: #07c160; color: #fff; border: none; border-radius: 4px; font-weight: bold; cursor: pointer; font-size: 15px; }
        ...
        .modal-box button { width: 100%; padding: 12px; background: #07c160; color: #fff; border: none; border-radius: 4px; font-weight: bold; font-size: 16px; cursor: pointer; }
    ...
    <div class="sidebar">
        ...
        <div class="sidebar-bottom">
            ...
            <div class="nav-tab" id="tab-settings" onclick="openSettings()">
    ...
    <div class="main-panel settings-container" id="settings-panel">
        ...
        function sleep(ms) {
        ...
        }function ensureSession(id, seed = {}) {
            if (!sessions[id]) {
                sessions[id] = {
                    id,
            ...
            list.innerHTML = '';
            let arr = Object.values(sessions).filter(s => s.id !== 'global');
            arr.sort((a, b) => {
                if (a.isPinned !== b.isPinned) return a.isPinned ? -1 : 1;
                if (a.online !== b.online) return a.online ? -1 : 1;
                return (b.lastActive || b.lastSeen || 0) - (a.lastActive || a.lastSeen || 0);
            });
            let allSessions = [sessions.global, ...arr];
            allSessions.forEach(s => {
                ...
                if (s.unread > 0) {
                    let unreadDot = document.createElement('span');
                ...
                if (s.id !== 'global') {
                    ...
                    pinBtn.innerHTML = '&#x1F4CC;';
                    pinBtn.title = s.isPinned ? '取消置顶' : '置顶';
                    pinBtn.onclick = (e) => {
                        e.stopPropagation();
                        pinBtn.classList.add('pin-anim');
                        setTimeout(() => {
                            s.isPinned = !s.isPinned;
                            if (s.isPinned && !s.pinTime) s.pinTime = Date.now();
                            renderSidebar();
                        }, 180);
                    };
            ...
            info.innerText = `大小: ${formatSize(meta.size || 0)}`;
            ...
            msgs.forEach(m => {
                ...
                if (!m.is_me) {
                    ...
                    if (currentSession === 'global') {
                        ...
                        priBtn.className = 'private-btn';
                        priBtn.textContent = '私聊';
                        priBtn.onclick = () => {
            ...
            const s = sessions[currentSession] || sessions.global;
            ...
            Object.values(sessions).forEach(s => {
                if (s.id !== 'global' && !seen.has(s.ip) && typeof s.online === 'boolean') {
                    s.online = false;
                }
            });
            ...
            messages.forEach(m => {
                let isGlobal = (m.target === '255.255.255.255' || m.target === '');
                ...
                let session = ensureSession(sessionId, {
                    name: initialName,
                    ip: sessionId,
                    online: !m.is_me ? true : (sessions[sessionId] ? sessions[sessionId].online : false),
                    lastActive: Date.now()
                });
                if (!m.is_me) {
            ...
            while (hasEntered) {
                try {
                    const res = await fetch(`/api/poll?cursor=${pollCursor}&msg_cursor=${msgCursor}`, { cache: 'no-store' });
                    const data = await res.json();
                    pollCursor = Number(data.cursor || pollCursor);
                    msgCursor = Number(data.msg_cursor || msgCursor);
            ...
            try {
                ...
                const dest = currentSession === 'global' ? '255.255.255.255' : sessions[currentSession].ip;
...
void handle_client(SOCKET c) {
        ...
        if (!header_parsed) {
            ...
            if (header_end != string::npos) {
                ...
                if (cl_pos != string::npos) {
                    ...
                    if (cl_end != string::npos) {string cl_str = req.substr(cl_pos + 15, cl_end - (cl_pos + 15));cl_str = trim_copy(cl_str);content_length = parse_int(cl_str, 0);
                    }
                }
            }
        }if (header_parsed) {
    ...
    }if (req.empty()) {API.cs(c);return;}string method = req.substr(0, req.find(' '));string path = get_request_path(req);string body = (header_end == string::npos) ? "" : req.substr(header_end + 4);
    if (method == "GET" && path == "/") {
        ...
        {
            unique_lock<mutex> lk(state_mtx);
            if (client_cursor > 0 && client_cursor >= state_cursor) {
                state_cv.wait_for(lk, chrono::milliseconds(LONG_POLL_TIMEOUT_MS), [&]() {
                    return state_cursor > client_cursor;
                });
            ...
            }for (const auto& kv : online_users) users.push_back(kv.second);
        }sort(users.begin(), users.end(), [](const OnlineUser& a, const OnlineUser& b) {
        ...
        for (size_t i = 0; i < messages.size(); ++i) {
            if (i) json += ",";
            const ChatMessage& m = messages[i];json += "{";json += "\"seq\":" + to_string(m.history_seq) + ",";json += "\"ip\":\"" + escape_json(m.sender_ip) + "\",";json += "\"name\":\"" + escape_json(m.sender_name) + "\",";json += "\"target\":\"" + escape_json(m.target_ip) + "\",";json += "\"content\":\"" + escape_json(m.content) + "\",";json += "\"is_me\":" + string(m.is_me ? "true" : "false");json += "}";
        }json += "],";json += "\"users\":[";
        ...
        string text = get_json_val(body, "text");string dest_ip = get_json_val(body, "ip");
        ...
        if (!new_name.empty()) {
            lock_guard<mutex> lk(state_mtx);my_name = new_name;
        ...
        if (!parse_multipart_file(content_type, body, filename, file_data)) {
            ...
            filename = basename_only(filename);string disk_name = generate_upload_name(filename);string file_path = upload_dir + "\\" + disk_name;ofstream ofs(file_path.c_str(), ios::binary);
            if (!ofs) {send_simple_response(c, "500 Internal Server Error", "application/json; charset=utf-8", "{\"error\":\"save failed\"}");}
                ...
                ofs.write(file_data.data(), (streamsize)file_data.size());ofs.close();
                {
                    ...
                    uploaded_files[disk_name] = {disk_name, filename, file_data.size()};
                }string json = "{";json += "\"name\":\"" + escape_json(filename) + "\",";json += "\"size\":" + to_string(file_data.size()) + ",";json += "\"url\":\"" + escape_json(http_base_url + "/uploads/" + disk_name) + "\"";json += "}";send_json_response(c, json);
            }
        }
        ...
        if (!is_safe_disk_name(disk_name)) {send_not_found(c);}
            ...
            string file_path = upload_dir + "\\" + disk_name;
            ifstream ifs(file_path.c_str(), ios::binary);
            if (!ifs) {send_not_found(c);}
            else {
                ifs.seekg(0, ios::end);
                ...
                }if (meta.original_name.empty()) meta.original_name = disk_name;
                ...
                while (ifs) {
                    ifs.read(file_buf, sizeof(file_buf));
                    streamsize chunk = ifs.gcount();
    ...
    srand((unsigned int)time(NULL) ^ GetCurrentProcessId());my_id = (rand() << 16) ^ (rand() ^ GetCurrentProcessId());my_name = "User_" + to_string(rand() % 9000 + 1000);system("chcp 936 > nul");API.init();
    WSADATA w;API.ws(MAKEWORD(2, 2), &w);CreateDirectoryA(upload_dir.c_str(), NULL);my_ip = detect_local_ip();http_base_url = "http://" + my_ip + ":28081";udp_socket = API.sk(AF_INET, SOCK_DGRAM, IPPROTO_UDP);int opt = 1;API.so(udp_socket, SOL_SOCKET, SO_BROADCAST, (char*)&opt, sizeof(opt));
    ...
    if (API.bd(udp_socket, (struct sockaddr*)&recv_addr, sizeof(recv_addr)) == SOCKET_ERROR) {
        cout << "[错误] 端口 8888 被占用！" << endl;Sleep(3000);return 1;
    }memset(&broadcast_addr, 0, sizeof(broadcast_addr));broadcast_addr.sin_family = AF_INET;broadcast_addr.sin_port = API.hs(8888);thread(udp_receiver).detach();thread(heartbeat_loop).detach();thread(retransmit_loop).detach();thread(maintenance_loop).detach();
    ...
    if (API.bd(tcp_socket, (sockaddr*)&web_addr, sizeof(web_addr)) == SOCKET_ERROR) {
        cout << "[错误] 端口 28081 被占用！" << endl;Sleep(3000);return 1;
